...
export interface IStorage {
  ...
  addTeamMemberByEmail(email: string, role: string): Promise<TeamMember>;
...
export class MemStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    const user: User = { 
      ...
      securityAnswer: hashedSecurityAnswer ?? null,  // Store the hashed security answer
  ...
  async getFilesByOwnerId(ownerId: number, includeDeleted: boolean = false): Promise<File[]> {
    const userFiles = Array.from(this.files.values()).filter(f => 
      f.ownerId === ownerId && (includeDeleted || !f.deletedAt)
    );
  ...
  async createFile(params: CreateFileParams): Promise<{
    ...
    const file: File = { 
      ...
      encryptionTag: params.encryptionTag ?? null,
      encryptionSalt: params.encryptionSalt ?? null,
  ...
  async getFolderContents(folderId: number): Promise<FolderContents> {
    ...
    const files = Array.from(this.files.values())
      .filter(file => file.folderId === folderId && !file.deletedAt);
  ...
  async getRootFolderContents(userId: number): Promise<FolderContents> {
    ...
    const files = Array.from(this.files.values())
      .filter(file => !file.folderId && file.ownerId === userId && !file.deletedAt);
  ...
  async permanentlyDeleteFile(fileId: number): Promise<boolean> {
    try {
      ...
      for (const [shareId, share] of Array.from(this.fileShares.entries())) {
  ...
  async addTeamMemberByEmail(email: string, role: string): Promise<TeamMember> {
    try {
  ...
  // Team activity methods
  async logDetailedActivity(params: {
    userId: number;
    action: string;
    resourceId: string;
    resourceType: string;
    ...
    const log: ActivityLog = {
      ...
      userId: params.userId,
      action: params.action,
      resourceId: parseInt(params.resourceId) || null,
